...
export class AstueEfficiencyUnitCardComponent implements OnChanges {
    ...
    public ngOnChanges(): void {
        ...
        // console.log(this.cardSelection.selected);
        // const changeFlows = this.unit.flows?.filter((x) => this.cardSelection.selected.some((s) => s.id === x.id));
        // changeFlows.forEach((x) => this.AsEfService.cardSelection.select(x));
    }
