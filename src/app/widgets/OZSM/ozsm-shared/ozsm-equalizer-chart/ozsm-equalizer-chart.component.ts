...
@Component({
    selector: 'evj-ozsm-equalizer-chart',
    templateUrl: './ozsm-equalizer-chart.component.html',
    styleUrls: ['./ozsm-equalizer-chart.component.scss'],
})
export class OzsmEqualizerChartComponent implements OnChanges, OnInit {
    @Input()
    ...
    private drawBackgroundCols(): void {
        ...
        this.svg.selectAll('.background-col line').attr('class', (d, g) => {
    ...
    private appendRect(x: number, y: number, className: string): void {
        this.svg
            .append('rect')
            .attr('x', this.scales.x(x) - 1) // смещаем rect для центрирования внутри колонки
