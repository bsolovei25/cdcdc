...
export class SystemPeriodEditComponent implements OnInit, OnChanges {
  ...
  putTemplate(): void {
    ...
    const windowsParam = {
      ...
      acceptFunction: () => {
        ...
        this.reportService.postSystemOptions(this.dataTemplate.id, obj).subscribe((ans) => {
  ...
  setValueTime(event): void { /// ВОЗМОЖНО НУЖНО БУДЕТ УБРАТЬ ( ВЫБОР ДАТЫ В ДАТА ПИКЕРЕ ЭВЕНТ )
