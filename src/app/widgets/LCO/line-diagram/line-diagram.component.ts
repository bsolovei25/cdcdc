...
import { ILineDiagramData, ILineDiagramDataItem } from '../../../dashboard/models/LCO/line-diagram';
            ...
            if (value.value > 0) {
                if (value.upperLimit > value.upperBound) {
                    value.percentFact =
                        ...
                        k2 *
                            (this.percentPlan +
                                ((value.value - value.upperBound) / (value.upperLimit - value.upperBound)) *
                                    this.percentDeviation);
                } else {
                    value.percentFact = 100;
                }
                value.percentFact = value.percentFact > 100 ? 100 : value.percentFact < 0 ? 0 : value.percentFact;
            } else {
                value.percentFact = 0;
            }
        });
