...
export class KpeChartsAnalyticBarChartComponent implements OnInit {
    @Input() public data: IKpeChartsAnalyticGraphData[];
    @Input() public interval: IInterval = {
        min: new Date(1628281600000), // 6 августа
        max: new Date(1630368000000), // 31 августа
    };
    ...
    public ngOnInit(): void {
        const graphData = this.data.filter(graph => graph.graphType === 'fact');
        const mappedGraphData = graphData[0].graph.map(point => {
            return {
        ...
        this.intervalHandler(mappedGraphData, this.interval);
        this.dataInInteval(mappedGraphData, this.interval);
    ...
    private dataInInteval(dataset: IBarChartDataset[], interval: IInterval): any {
        if (
            dataset.length === 0 ||
